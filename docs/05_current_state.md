...
- `PlayerDataService` - профиль игрока, загрузка/сохранение через DataStore, публичные данные для UI; хранит `CapturedCamps`, `CampOutposts` и `JoinedNPCs`.
...
- `CombatService` - меч-`Tool`, серверный hitbox, враги с HP, тренировочный манекен, враждебный лагерь, захват территории, маркер и постройка аванпоста, восстановление захвата по профилю, спасённый NPC и его вербовка в лагерь (поле `JoinedNPCs`), работник лагеря `CampWorker_BanditCamp_01` с тегом «Житель лагеря» и статус-промптом `CampWorkerStatusPrompt`, его восстановление после Stop -> Play и перенос к аванпосту.
...
NPC-работников: пока без пассивного дохода, автоматизации и профессий. Следующий шаг
развития — Фаза 2.3 (минимальная серверная заглушка назначения работнику, без производства),
см. `docs/planning/05_next_codex_task.md` и мастер-план
