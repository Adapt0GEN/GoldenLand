...
## Фаза 2.3: заглушка назначения работнику лагеря

Фаза 2.3 реализована в `CombatService` и `PlayerDataService` и протестирована. Это
следующий маленький шаг Фазы 2: работник лагеря получает первое сохранённое
placeholder-задание. Это всё ещё фундамент будущей автоматизации — без реального
производства, дохода или системы профессий.

### Сохранённое поле назначения

- В профиль добавлено новое сохраняемое поле `WorkerAssignments = {}` (таблица по id
  лагеря, по образцу `JoinedNPCs`/`CapturedCamps`/`CampOutposts`).
- Поле добавлено во всех трёх местах `PlayerDataService`: дефолтный профиль,
  нормализация загруженного профиля и сериализация при сохранении.
- Старые сейвы без `WorkerAssignments` безопасно грузятся как пустая таблица `{}`,
  без `nil`-индексации. Существующие сейвы с `JoinedNPCs`, `CapturedCamps` и
  `CampOutposts` продолжают работать.
- `WorkerAssignments` сохраняется и восстанавливается после Stop -> Play.

### Поведение статус-промпта работника

- Работник `CampWorker_BanditCamp_01` использует существующий промпт
  `CampWorkerStatusPrompt` из Фазы 2.2 (новый промпт не добавлялся).
- При первом разговоре сервер ставит `profile.WorkerAssignments["BanditCamp_01"] = "Idle"`,
  помечает профиль грязным существующим паттерном `MarkDirty`, сохраняет через
  `SaveProfile` и шлёт сообщение «Житель назначен в лагерь. Задания появятся позже.».
- При повторном разговоре состояние не дублируется: сервер шлёт сообщение
  «Житель лагеря: текущее задание — ожидание.».
- После Stop -> Play назначение сохраняется, и повторный разговор показывает уже
  присвоенное задание.

### Без экономики

- Взаимодействие с работником не начисляет и не списывает ресурсы
  (`Gold`, `Wood`, `Stone`, `Metal`, `MetalIngot`, `MetalParts`), не запускает
  производство, таймеры или циклы и не даёт пассивного дохода.

### Server-authoritative

- Все изменения состояния происходят на сервере. Клиент только триггерит
  `ProximityPrompt`; сервер проверяет профиль, пишет `WorkerAssignments` и шлёт
  сообщение. Клиентские значения назначения не принимаются на веру.

### Диагностические логи

- Первое назначение: `[CombatService] PlayerName assigned camp worker at BanditCamp_01 to Idle.`
- Повторная проверка: `[CombatService] PlayerName checked camp worker assignment at BanditCamp_01: Idle.`

### Что ещё НЕ сделано в Фазе 2.3

- Нет пассивного дохода.
- Нет производства по таймеру.
- Нет полной автоматизации работников.
- Нет настоящей системы профессий/работ (только placeholder-роль `Idle`).
- Это только фундаментальный шаг под будущую автоматизацию.

## Текущие работающие системы

- `PlayerDataService` - профиль игрока, загрузка/сохранение через DataStore, публичные данные для UI; хранит `CapturedCamps`, `CampOutposts`, `JoinedNPCs` и `WorkerAssignments`.
...
- `CombatService` - меч-`Tool`, серверный hitbox, враги с HP, тренировочный манекен, враждебный лагерь, захват территории, маркер и постройка аванпоста, восстановление захвата по профилю, спасённый NPC и его вербовка в лагерь (поле `JoinedNPCs`), работник лагеря `CampWorker_BanditCamp_01` с тегом «Житель лагеря» и статус-промптом `CampWorkerStatusPrompt`, его восстановление после Stop -> Play и перенос к аванпосту, а также заглушка назначения работнику (`WorkerAssignments[campId] = "Idle"`) без экономики.
...
- цепочка `спасённый NPC -> работник лагеря -> заглушка задания Idle`: сохранённое поле `WorkerAssignments`, назначение `Idle` при первом разговоре и его восстановление после Stop -> Play без изменения ресурсов.
...
(минимальная серверная заглушка назначения работнику `WorkerAssignments[campId] = "Idle"`,
без производства и экономики) завершена и протестирована. Следующий рекомендованный
шаг — не новая геймплейная система, а архитектурный/планировочный проход для
определения границ сервисов перед Фазой 2.4, см. `docs/planning/05_next_codex_task.md`
и мастер-план в `docs/planning/16_assessment_and_master_plan.md`. Отложенный мелкий шаг
кузницы (Forge level 2 за `MetalParts`) остаётся как опциональная полировка экономики.

## Известные ограничения и архитектурные заметки

К концу Фазы 2.3 накопились архитектурные долги, которые важно учитывать перед
добавлением новых геймплейных систем:

- `CombatService` теперь владеет слишком многим: бой, враждебный лагерь,
  восстановление захваченного лагеря, спасённый NPC, визуал работника лагеря и
  заглушка назначения работнику. Для MVP это приемлемо, но позже сервис стоит
  разделить на более узкие сервисы.
- `PlotService` остаётся очень большим и тоже должен быть разбит позже.
- Визуалы лагеря и работника по-прежнему общие объекты `Workspace`, а состояние
  (`CapturedCamps`, `CampOutposts`, `JoinedNPCs`, `WorkerAssignments`) хранится в
  профиле конкретного игрока. Для соло-MVP это приемлемо, но перед мультиплеером
  (Фаза 3) такую модель владения нужно пересмотреть.
- Не следует добавлять пассивный доход или автоматизацию до архитектурного прохода.
- Рекомендованный следующий шаг — архитектурный/планировочный проход для определения
  границ сервисов перед продолжением (Фаза 2.4).
