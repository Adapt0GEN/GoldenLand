...
- `WorldService` — публичная точка входа мира: стартовая сцена, гейты зон, визуалы
  ForestZone и RockZone и их состояния; читает `ForestZoneState` и рисует визуал.
  Владеет визуальным рендером, а не данными ресурсной зоны; runtime `ForestZoneState`
  не пишет. Generic-помощники построения мира вынесены в `Services/World/`
  (см. раздел «Декомпозиция WorldService»); `WorldService` остаётся координатором и
  владеет интеграцией с профилем и `ResourceService`.
...
## Декомпозиция WorldService (World/ helper-модули)

`WorldService.lua` был разбит из крупного монолитного файла построения мира в более
компактную публичную точку входа. Поведение мира при этом сохранено без изменений.

- `WorldService.lua` остаётся публичным сервисом мира: создание стартовой сцены,
  обновление доступа к зонам для игрока, чтение состояния `ForestZoneState`,
  интеграция с профилем и `ResourceService`.
- Generic-помощники построения мира теперь живут в
  `src/ServerScriptService/Services/World/`:
  - `WorldLayoutConfig.lua` — статические константы раскладки (имена, координаты зон
    и точек, общие наборы); игрового состояния не хранит.
  - `WorldPartFactory.lua` — базовые помощники создания Part/Folder и простые
    визуальные свойства; без зон-специфичной логики.
  - `WorldSignBuilder.lua` — знаки, таблички, доски и текстовые маркеры.
  - `WorldZoneBuilder.lua` — визуал ForestZone и RockZone, состояния визуала леса и
    декоративные объекты зон.
  - `WorldPathBuilder.lua` — заблокированные пути и проходы (`BlockedPathToForest`,
    `RockPass`); обработчик расчистки передаётся через колбэк.
- ForestZone, RockZone, `RockPass`, заблокированный путь в лес и визуальные
  построители мира теперь опираются на эти helper-модули.
- Имена объектов, имена промптов, тексты знаков и строки логов сохранены.

Этот рефакторинг подготавливает проект к следующему игровому шагу — первой
читаемой раскладке маршрута острова. Планируемое направление маршрута:
...
- RockPass / RockZone;
- BanditCamp path;
- BanditCamp_01 / захваченный аванпост.

Будущую работу над картой следует вести через `World/` builders/config, а не
возвращать большие блоки построения обратно в `WorldService.lua`.
